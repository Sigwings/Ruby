...
title: 'Test Report',
display: false,
email: ['joelpearson@centrexservices.co.uk'],
options: [ 
...
class Reporter < RDT
    ...
    @args[:key].each { |jobref|
      ...
      if @args[:special] != 'PO|L' || @driver.text_field( :name => 'custordref' ).value != jobref.split('|').first
        ...
        when 'Workshop Reports'
          ...
          @driver.select_list( :id => 'berhandle' ).select( 'are always shown' ) if @args[:ber]
        ...
        save_labels( jobref, true) if get_labels
  ...
  def save_labels( jobref, printlabels = false )
    ...
    wait_for_download
    print_label( @label_file ) if printlabels
  ...
  def output_data( output = nil, input_array = false, display = nil )
    ...
    if output =~ /\.xlsx/i
      ...
      begin
        ...
        sht = wb.sheets(1)
        sht.name = 'Report'
        sht.range( sht.cells( 1, 1 ),sht.cells( input_array.length, input_array[0].length ) ).value = input_array
      ...
      outputstring = ''
      @data.each { |ar| outputstring << "#{ ar.join("\t") }\n" }
  ...
  def output_to_excel_tabs(input_hash = { 'tab_name' => 'Array of Arrays' }, filename = 'ReportOutput.xlsx', display = nil )
    ...
    begin
      ...
      wb = excel.Workbooks.Add
      ( wb.sheets.count - 1 ).times { wb.sheets(2).delete }
      ...
      input_hash.each do |name, data|
        ...
        sht.name = "#{name}"
        sht.range( sht.cells( 1, 1 ), sht.cells( data.length, data[0].length ) ).value = data
        sht.cells.EntireColumn.AutoFit
  ...
  def send_email(attachments=[], subject_var=nil, to_ary=nil, body_var='Automated email')
    ( @args[:email].nil? ? to_ary = 'joelpearson@centrexservices.co.uk' : to_ary = @args[:email] ) if to_ary.nil?
    ...
    options = { 
      ...
      :port                  => 587,
      :user_name             => 'automatic.emailer.centrex@gmail.com',
      :password              => 'centrex01',
